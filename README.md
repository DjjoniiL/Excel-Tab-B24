...
`Excel таблица в сделке и экспорт MVP Final v.34` - статическое serverless-приложение для Bitrix24 Marketplace. Оно добавляет в карточку CRM-сделки вкладку с Excel-подобной таблицей, где пользователь может вести рабочие расчеты по сделке, подтягивать поля CRM, считать формулы, форматировать ячейки и выгружать заполненную область в Excel.
