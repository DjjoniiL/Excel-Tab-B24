...
```text
Excel таблицы в сделке и экспорт данных из CRM v.46
```

v46 зафиксирована как релизная версия после пользовательской проверки: таблица, добавление/удаление строк и столбцов, перенос строк и столбцов, формулы, форматирование, выделение, CRM-привязки и новые сделки работают корректно.
...
Скрипт `npm run package` не перезаписывает существующий zip. Для повторной сборки v46 нужно вручную удалить старый архив или поднять номер версии в `tools/build-marketplace-zip.ps1`.
...
- `RELEASE_REPORT_v46.md` - итоговый отчет по релизу.
